...
const TICKERS = {
  ...
  'Доллар к рублю':'USD000UTSTOM','Юань к рублю':'CNYRUB_TOM'
};
    ...
    if(r.ok){
      ...
      for(const key of Object.keys(FUT_ASSETS)){
        ...
        const rows = b.data
          ...
          .map(r2 => ({ sec:r2[iSec], name:r2[iNam], exp:new Date(r2[iExp]).getTime() }))
...
async function news(topic, extra){
  ...
  if(list.length < 3){ list = win(24); note = 'за последние 24 часа'; }
  if(list.length < 3){ list = match;   note = 'свежих новостей по теме мало, лента без ограничения по времени'; }
...
const fmt = q => {
  if(q.v==null) return `${q.name}: данных нет — в тексте НЕ упоминать`;
  if(q.stale)   return `$${q.t} — ${q.name}: свежей цены нет, торги приостановлены или идёт дискретный аукцион. ${q.v} это ЗАКРЫТИЕ ПРЕДЫДУЩЕГО ДНЯ, НЕ текущая цена. Так и написать. Тикер в посте: $${q.t}`;
  return `$${q.t} — ${q.name}: ${q.v}${q.c==null?'':', изменение '+q.c+'%'}. Тикер в посте: $${q.t}`;
};
...
ПРАВИЛО НОМЕР ОДИН, ВАЖНЕЕ ВСЕХ ОСТАЛЬНЫХ — ТИКЕРЫ.
Каждый инструмент, о котором ты пишешь, ОБЯЗАН быть назван вместе с тикером.
Тикер уже дан тебе готовым в блоке котировок, в конце каждой строки написано «Тикер в посте: $XXXX». Копируй его буква в букву.
Формат: знак доллара, латиница, без скобок и без пробела — $TATN, $BRU6, $SBER.
Тикер ставится при ПЕРВОМ упоминании инструмента в посте, сразу после названия: «Татнефть $TATN», «нефть Brent $BRU6».
Если инструмента нет ни в котировках, ни в таблице внизу — писать без тикера. НИКОГДА не сочинять тикер сам и не переводить русское название в латиницу своими силами. «$Система» или «$Neft» — грубая ошибка.
Перед тем как отдать ответ, перечитай свой текст и проверь: у каждого названного инструмента стоит тикер из данных.
...
Подзаголовки:
— Придумывать заново под конкретную новость, живым языком.
...
СПРАВОЧНАЯ ТАБЛИЦА ТИКЕРОВ (на случай, если упоминаешь бумагу, которой нет в котировках):
${TICKER_TABLE}`;
...
Котировки Московской биржи (задержка 15 минут). В конце каждой строки указан тикер, который надо поставить в текст:
${quotes}

Заголовки новостей ${feedText}

Напоминание: у каждого инструмента в тексте должен стоять тикер со знаком доллара из строк выше.
